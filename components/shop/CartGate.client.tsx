...
import { removeFromCart, processCheckout } from "@/lib/actions/shop";
...
export default function CartGate({
  ...
  useEffect(() => {
    ...
    setCurrentUserId(initialUserId ?? null);
  }, [initialItems, initialBalance, initialUserId]);
    ...
    <>
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 border border-dashed border-white/10 rounded-3xl bg-[#0a0a0a]">
          ...
          <h3 className="text-xl text-white font-bold mb-2">Túi trống rỗng!</h3>
