...
const DecayCard: React.FC<DecayCardProps> = ({
  width = 300,
  height = 400,
  image = "/cartethiya-fleurdelys-v0-jhbewa3tmyre1.webp",
  children,
}) => {
  const svgRef = useRef<HTMLDivElement | null>(null);
  const displacementMapRef = useRef<SVGFEDisplacementMapElement | null>(null);

  // --- FIX QUAN TRỌNG: Tạo ID duy nhất cho mỗi instance của Card ---
  const uniqueId = useId();
  const filterId = `imgFilter-${uniqueId}`;

  const cursor = useRef<{ x: number; y: number }>({
    x: 0, // Khởi tạo 0 để tránh lỗi server-side mismatch
    y: 0,
  });
  ...
  useEffect(() => {
    // Cập nhật lại giá trị ban đầu khi mount
    winsize.current = {
  ...
  return (
    ...
    >
      ...
      >
        {/* Sử dụng filterId động thay vì cứng "imgFilter" */}
        <filter id={filterId}>
          <feTurbulence
            type="turbulence"
            baseFrequency="0.015"
