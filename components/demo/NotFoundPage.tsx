"use client";

import React, { useEffect, useRef } from "react";
import Link from "next/link";
import "./not-found.css"; // Import file CSS vừa tạo

export default function NotFoundPage() {
  const imgRef = useRef<HTMLImageElement | null>(null);
  const lFollow = useRef({ x: 0, y: 0 });
  const pos = useRef({ x: 0, y: 0 });
  const containerRef = useRef<HTMLDivElement>(null); // Ref để giới hạn phạm vi bắt sự kiện chuột

  useEffect(() => {
    let animationFrame = 0;
    const friction = 1 / 30;

    function animate() {
      pos.current.x += (lFollow.current.x - pos.current.x) * friction;
      pos.current.y += (lFollow.current.y - pos.current.y) * friction;

      const translate = `translate(${pos.current.x}px, ${pos.current.y}px) scale(1.1)`;
      if (imgRef.current) {
        imgRef.current.style.transform = translate;
      }
      animationFrame = window.requestAnimationFrame(animate);
    }

    function onMove(e: MouseEvent) {
      // Tính toán dựa trên kích thước của container demo thay vì window toàn cục
      if (!containerRef.current) return;

      const rect = containerRef.current.getBoundingClientRect();
      const centerX = rect.width / 2;
      const centerY = rect.height / 2;

      // Tọa độ chuột tương đối trong container
      const mouseX = e.clientX - rect.left;
      const mouseY = e.clientY - rect.top;

      const lMouseX = Math.max(-100, Math.min(100, centerX - mouseX));
      const lMouseY = Math.max(-100, Math.min(100, centerY - mouseY));

      lFollow.current.x = (20 * lMouseX) / 100; // horizontal follow
      lFollow.current.y = (10 * lMouseY) / 100; // vertical follow
    }

    // Gắn sự kiện vào container thay vì window để hiệu ứng chỉ chạy khi hover vào demo
    const container = containerRef.current;
    if (container) {
      container.addEventListener("mousemove", onMove);
      container.addEventListener("click", onMove);
    }

    animate();

    return () => {
      if (container) {
        container.removeEventListener("mousemove", onMove);
        container.removeEventListener("click", onMove);
      }
      window.cancelAnimationFrame(animationFrame);
    };
  }, []);

  return (
    // Wrapper div để cô lập styles và events
    <div
      className="not-found-wrapper w-full h-[600px] relative bg-[#202239] overflow-hidden"
      ref={containerRef}
    >
      <div className="page">
        {/* Logo SVG Symbol Definition */}
        <svg style={{ display: "none" }} xmlns="http://www.w3.org/2000/svg">
          <symbol id="logo-dailyui" viewBox="0 0 61 14">
            <g>
              <g>
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  fill="#525C7C"
                  d="M29.117,0.98h-1.846l-2.407,11.551h6.045l0.334-1.62h-4.198
      L29.117,0.98z M40.132,0.98l-3.365,5.285l-1.2-5.285h-1.885l1.854,7.182l-0.926,4.369h1.854l0.889-4.337l4.853-7.214H40.132z
       M8.529,2.125C7.771,1.362,6.688,0.98,5.282,0.98H2.407L0,12.531h3.124c1.339,0,2.505-0.286,3.497-0.861
      c0.992-0.573,1.748-1.392,2.267-2.457c0.52-1.063,0.779-2.309,0.779-3.737C9.667,4.006,9.288,2.889,8.529,2.125z M7.201,8.351
      c-0.366,0.821-0.892,1.46-1.577,1.913c-0.686,0.452-1.488,0.679-2.407,0.679H2.166L3.934,2.56h1.137
      c0.873,0,1.537,0.254,1.994,0.762S7.751,4.57,7.751,5.539C7.751,6.592,7.567,7.53,7.201,8.351z M20.128,12.531h1.831L24.382,0.98
      H22.55L20.128,12.531z M15.314,0.964l-6.1,11.567h1.979l1.627-3.208H16.6l0.319,3.208h1.823L17.448,0.964H15.314z M13.632,7.68
      l1.339-2.655c0.452-0.885,0.821-1.693,1.106-2.425c0,0.274,0.015,0.642,0.043,1.102c0.029,0.461,0.147,1.787,0.354,3.979H13.632z
       M44.33,0l-2.662,14h16.67L61,0H44.33z M52.584,8.584c-0.265,1.279-0.755,2.229-1.473,2.849c-0.719,0.62-1.67,0.931-2.854,0.931
      c-1.025,0-1.816-0.254-2.376-0.762c-0.561-0.508-0.839-1.236-0.839-2.19c0-0.397,0.047-0.805,0.141-1.222l1.367-6.473h1.679
      l-1.359,6.501c-0.1,0.422-0.149,0.801-0.149,1.136c0,0.488,0.141,0.867,0.422,1.139c0.28,0.271,0.707,0.405,1.277,0.405
      c0.685,0,1.229-0.192,1.63-0.577c0.401-0.386,0.694-1.013,0.879-1.88l1.408-6.724h1.68L52.584,8.584z M56.034,12.221h-1.665
      l2.203-10.504h1.664L56.034,12.221z"
                />
              </g>
            </g>
          </symbol>
        </svg>

        <header>
          <div className="logo">
            <svg>
              <use xlinkHref="#logo-dailyui" />
            </svg>
          </div>
          <nav />
          <div className="search">
            {/* Giả lập icon search bằng SVG đơn giản */}
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            >
              <circle cx="11" cy="11" r="8"></circle>
              <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
            </svg>
          </div>
        </header>

        <div className="content">
          <h1>404</h1>
          <h2>Page not found</h2>
          <p>I tried to catch some fog, but I mist</p>
          <Link href="#" className="back">
            back to home
          </Link>
        </div>

        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          ref={imgRef}
          className="bg-img"
          src="http://www.supah.it/dribbble/008/008.jpg"
          alt="bg"
        />
      </div>
    </div>
  );
}
