...
import IsolatedInput from "@/components/ui/IsolatedInput.client";
...
export default function ChatPageClient({
  ...
  const [isChecking, setIsChecking] = useState<boolean>(initialChecking);
  ...
  const handleSubmit = async (e?: React.FormEvent, txt?: string) => {
    ...
    setInput("");
    // clear uncontrolled input value
    try {
  ...
  return (
    <div className="fixed top-[64px] left-0 right-0 bottom-0 bg-[#050505] text-slate-200 font-mono z-40 flex overflow-hidden">
      <style jsx>{`
        /* small subset of styles kept inline to preserve exact UI */
      `}</style>
