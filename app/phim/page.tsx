...
// Giả lập role hiện tại của user (Đổi thành 'Chí Tôn' để test mở khóa)

// --- 1. OPTIMIZED SPIRIT CURSOR (Fix lỗi giật lag) ---

// Server-rendered CultivationBar structure. Client script updates active state and fill height.
...
export default function VanTuongDaiHoanMy() {
  const categories = [
  ...
  return (
    <div className="relative min-h-screen bg-[#050505] text-[#e5e5e5] font-serif overflow-x-hidden selection:bg-[#b45309]/40 selection:text-[#ffd700]">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        {/* Noise Texture cho cảm giác giấy cũ/cổ kính */}

        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-10" />

        {/* Tinh vân xoay chậm */}

        <div className="absolute top-[-50%] left-[-20%] w-[150%] h-[150%] bg-[radial-gradient(circle,rgba(20,20,30,1)_0%,rgba(0,0,0,0)_70%)] opacity-60 animate-pulse-slow" />

        {/* Vòng Bát Quái Xoay (Rất mờ làm nền) */}

        <div className="absolute top-[20%] right-[-10%] w-[800px] h-[800px] border border-white/5 rounded-full flex items-center justify-center opacity-5 animate-[spin_120s_linear_infinite]">
      ...
      <header className="relative w-full h-[90vh] flex flex-col items-center justify-center overflow-hidden z-10">
        ...
        <div
          className="absolute inset-0 bg-[url('/film/unnamed (1).jpg')] bg-cover bg-center transition-transform duration-100 ease-linear"
          style={{
        ...
        <div
          className={`relative z-20 text-center transition-all duration-1000 opacity-100 translate-y-0`}
        >
          ...
          <div className="mt-12">
            <button
              data-scroll-to="next"
              className="group relative px-12 py-4 bg-transparent border border-[#ffd700]/30 text-[#ffd700] hover:bg-[#ffd700]/10 transition-all duration-500 overflow-hidden"
            >
      ...
      {/* === 2. SECTION: VẠN CỔ HỌA QUYỀN (FEATURED SCROLL) - Đã tách khỏi Footer === */}

      {/* Đây là vị trí "hợp phong thủy" nhất cho SpiritScroll: Ngay sau Hero, trước Grid */}
      ...
      <section className="relative z-20 w-full py-16 bg-[#080808] border-b border-stone-800">
        ...
        {/* Spirit Scroll Component đặt ở đây sẽ không bị che và hiển thị đẹp nhất */}

        <div className="w-full relative">
          <SpiritScroll />

          {/* Overlay 2 bên để làm mờ cạnh cuộn */}
        ...
        >
          {TIEN_HIEP_DATABASE.map((item, index) => (
            <div
              ...
              className={`group relative w-full h-[480px] mx-auto cursor-pointer perspective-1000 opacity-100 translate-y-0`}
              style={{ transitionDelay: `${index * 50}ms` }}
            >
              {/* Card Container với 3D hover */}

              <div className="relative w-full h-full transition-transform duration-500 ease-out group-hover:[transform:rotateX(5deg)_rotateY(5deg)_translateY(-10px)] [transform-style:preserve-3d]">
                ...
                <div className="absolute inset-0 bg-[#0f0f0f] border border-stone-800 rounded-lg overflow-hidden shadow-xl group-hover:border-[#ffd700]/40 group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.8)] transition-all duration-500">
                  ...
                  <Image
                    ...
                    alt={item.title}
                    fill
                    className="object-cover opacity-50 grayscale group-hover:grayscale-0 group-hover:opacity-30 group-hover:scale-110 transition-all duration-700"
                  />
                ...
                <div className="absolute bottom-0 left-0 right-0 h-full pointer-events-none z-30 [transform-style:preserve-3d] overflow-visible">
                  <div className="relative w-full h-full">
                    <img
                      ...
                      alt="Character"
                      className="absolute bottom-0 left-1/2 -translate-x-1/2 max-w-[150%] max-h-[120%] object-contain filter brightness-75 contrast-125 [transform:translateZ(50px)_translateY(20px)_scale(0.9)] group-hover:[transform:translateZ(80px)_translateY(-15px)_scale(1.05)] group-hover:brightness-110 group-hover:drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] transition-all duration-500 ease-out will-change-transform"
                    />

                    {/* Hào quang dưới chân */}

                    <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[60%] h-8 bg-[#ffd700]/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  </div>
                </div>

                {/* 4. BADGE TÔNG MÔN (Góc phải trên) */}
      ...
      {/* Global styles moved to `app/globals.css` to avoid styled-jsx in Server Component */}
    </div>
  );
}
